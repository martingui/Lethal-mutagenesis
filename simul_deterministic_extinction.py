...
for UU in range(3,24,2):
    print('U='+str(UU))
    Lambda_ode=0.001
    while Lambda_ode < 4.002 and Lambda_ode<=prev:
        ...
        sigma0=math.sqrt(0.25) #SD , math.sqrt(variance)  STANDING VARIANCE
        g0=0
...
plt.figure()
plt.scatter(X,Y,color="k",facecolors="none")
plt.scatter(lambdas,Us,c=cols)
plt.savefig("/Users/martin/Documents/lethal/figures/Uc_regimes/simul_deter_2025_lambda.png",dpi=600)

# Save data (adjust paths)
data = np.column_stack((lambdas, Us, yesno))
np.savetxt("/Users/martin/Documents/lethal/figures/Uc_regimes/simul_deter_2025_lambda.csv", data, delimiter=",", header="lambda,U,Is", comments="")
