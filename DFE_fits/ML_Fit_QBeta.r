...
QB0 <- read_delim("data_txt_files/QBeta_single_mutations.txt", 
                                          delim = "\t", escape_double = FALSE, 
                                          trim_ws = TRUE)

nmut=dim(QB0)[1] # number of mutations in dataset

# P(lethal) = f(type) (type: synonymous (S), non-synonymous (NS), stop codon (STOP), intergenic (I)
viabilities=ftable(QB0[,c('type','viable')])
# no significant dependence of type and viability 
fisher.test(viabilities)

# => estimate P(lethal) over all mutations: 28%
Plethal=sum(viabilities[,1])/sum(viabilities) 
# CI for P(lethal) based on binomial sample [16%-42%]
...
plot(abs(QB$y),QB$SEMy^2,log='',xlab='|y|',ylab='Ve(y)',pch=16,col='red')
abline(h=mean(QB$SEMy),lty='dashed')
# no significant heteroscedasticity found (SEM² vs |s|)
hetTest=lm(QB$SEMy^2 ~ abs(QB$y))
summary(hetTest)
# so no need for heteroscedastic MLE here

#remove 2 OUTLIERS with extreme error variance
QBc=QB[order(-QB$SEMy),][-(1:2),]
lines(abs(QBc$y),QBc$SEMy^2,col='blue',type='p',pch=16)
abline(h=mean(QBc$SEMy^2),lty='dashed',col='blue')

# change rescaling after removing OUTLIER
Esc=abs(mean(QBc$s))
QBc$y=QBc$s/Esc
QBc$SEMy=QBc$SEM/Esc

#----------- MLE exploration for QB (sanjuan 2004) ----------------------------------------
y0s=seq(from=0,to=20*max(QBc$s),length.out=100)
ns=1:10;
MLEs=MLEygrid(QBc,ns,y0s)  #compute loglik profile
...
ggsave("QB_LL_profiles.pdf")


#LRT for y0 = 0: non significant y0
X=2*(LLobs(QBc,best$n,best$y0)-LLobs(QBc,best$n,0))
1-pchisq(X,df=1)
...
llf<-function(par){
  logy0<-par[1]
  a<-par[2]
  return(LLsobs(QBc,best$n,1E-8+exp(logy0),Esc*exp(a)))
  }
...
llf<-function(par){
  logn<-par[1]
  a<-par[2]
  return(LLsobs(QBc,1.001+exp(logn),0,Esc*exp(a)))
}
...
View(estimates)
write.table(estimates,file="QB_estimates.txt")
